...
function loadSnapshot() {
    ...
    throw new Error(`Could not load bundled snapshot.json. Looked in: ${candidates.join(", ")}`);
}
...
const InputSchema = {
    ...
    model: z
        ...
        .describe('Model id as it appears in the snapshot, e.g. "claude-sonnet-4-6" (anthropic) or "gpt-5.4" (openai). An invalid id returns an error listing valid models.'),
    tier: z
        .string()
        .min(1, "tier is required")
        .describe('Rate-limit tier. Anthropic: "t1" or "t4". OpenAI: "t1" or "t5". An invalid tier returns an error listing valid tiers for the chosen provider.'),
    rpm: z
        .number()
...
const OutputSchema = {
    monthly_cost: z
        .number()
        .describe("Projected 30-day spend in USD, rounded to 2 decimals. = cost_per_request * rpm * 60 * 24 * 30."),
    monthly_cost_formatted: z
        .string()
        .describe('Same value as a human string, e.g. "$349,920.00".'),
    first_binding_429_dim: z
        .string()
        .describe("The rate-limit dimension that hits its ceiling first (highest utilization). RPM/ITPM/OTPM for anthropic; RPM/TPM for openai. This is what 429s before anything else when you scale up."),
    headroom_per_dim: z
        .record(z.string(), z.number())
        .describe("limit - usage for each dimension (per minute). Negative means you are already over that limit and will 429."),
    util_per_dim: z
        .record(z.string(), z.number())
        .describe("usage / limit * 100 for each dimension (percent, full precision). >100 means over the limit."),
    will_429: z
        .boolean()
        .describe("True if ANY dimension's projected usage exceeds its limit (utilization > 100%) at the given rpm/token profile."),
    snapshot_version: z
        .string()
        .describe('Date-stamped version of the pricing/rate-limit data used, e.g. "2026-05-15". Numbers are only as fresh as this snapshot.'),
};
...
function validModels(provider) {
    return Object.keys(SNAPSHOT.providers[provider].models);
}
function validTiers(provider) {
    if (provider === "anthropic") {
        ...
        for (const m of Object.keys(perModel)) {
            for (const t of Object.keys(perModel[m]))
                set.add(t);
        }
        return [...set];
    }
...
function tiersForModel(provider, model) {
    if (provider === "anthropic") {
...
function computePlan(provider, model, tier, rpm, inTok, outTok) {
    ...
    for (const d of included) {
        if (!binding || d.util > binding.util)
            binding = d;
    }
    const headroom_per_dim = {};
    const util_per_dim = {};
    for (const d of included) {
...
function renderText(provider, model, tier, r) {
...
server.registerTool("llm_capacity_plan", {
...
}, async ({ provider, model, tier, rpm, in_tok, out_tok }) => {
    const p = provider;
    // Validate model.
...
async function main() {
    ...
    console.error(`llmcapplanner-mcp-server v0.1.0 listening on stdio (snapshot ${SNAPSHOT.version})`);
}
main().catch((err) => {
